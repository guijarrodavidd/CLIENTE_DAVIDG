window.onload = function () {
  ...
  drawAllPlayers(players);
  let posArray = players.map((player) => player.position); // usamos .map para sacar TODAS las posiciones de TODOS los jugadores con funcion flecha
  var posSet = new Set(posArray); // .map pasa todo incluso repetido, entonces para quitar los repetidos tenemos que pasarselo como Set
  posArray = Array.from(posSet).sort(); // lo volvemos a pasar como array porque set tiene otra estructura y lo ordenamos por A-Z.

  let posDropdown = document.createElement("select"); //creamos elemento select para hacerlo dinamico
  posDropdown.id = "f_position"; // le aplicamos id
  ...
  function applyAllFilters() {
    ...
    // Filter by PPG
    // filteredPlayers = filteredPlayers.filter((player) => {
    //   intpPpg = parseFloat(pPpg.value);
    //   console.log(intpPpg, pPpg.value);
      
    //   return player.pointsPerGame == pPpg.value;
    // });
  ...
  let pName = this.document.querySelector("#f_name");
  ...
  // Remove all filters
  let remFilBtn = document.querySelector("#removeFilters");
  remFilBtn.onclick = function (e) {
    cleanContainer();
    drawAllPlayers(players);
    blankOpt.selected = true;
    pName.value = "";
